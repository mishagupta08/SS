...
" FROM MemberProfiles a LEFT JOIN Memberlogic b ON a.RegID = b.RegID LEFT JOIN MemberAddress c ON a.RegID = c.RegID WHERE a.RegID = @IdNo";
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                objCustomerDetail.IdNo = reader["FormNo"] != null ? reader["FormNo"].ToString() : "";
                                objCustomerDetail.ReferenceIdNo = reader["RefId"] != null ? reader["RefId"].ToString() : "";
                                objCustomerDetail.ReferenceName = reader["RefName"] != null ? reader["RefName"].ToString() : "";
                                objCustomerDetail.Name = reader["Name"] != null ? reader["Name"].ToString() : "";
                                objCustomerDetail.Address = "";// reader["Address"] != null ? reader["Address"].ToString() : "";
                                objCustomerDetail.FormNo = reader["FormNo"] != null ? decimal.Parse(reader["FormNo"].ToString()) : 0;
                                objCustomerDetail.StateCode = 0;// reader["StateCode"] != null ? decimal.Parse(reader["StateCode"].ToString()) : 0;
                                objCustomerDetail.MobileNo = reader["Mobl"] != null ? reader["Mobl"].ToString() : "";
                                objCustomerDetail.KitId = 0;// reader["KitId"] != null ? int.Parse(reader["KitId"].ToString()) : 0;
                                ...
                                if (reader["ActiveStatus"].ToString() != "Y")
                                {
                        ...
                        if (objCustomerDetail != null)
                        {
                            ...
                            query = "Select * FROM [GetBalance]('" + objCustomerDetail.FormNo + "','')";
                            cmd = new SqlCommand();
                            cmd.CommandText = query;
                            //cmd.Parameters.AddWithValue("@IdNo", IdNo);
                            cmd.Connection = SC;
                            ...
                            //         query = "Select Min(KitAmount) KitAmount FROM " + db + "..M_KitMaster WHERE KitAmount>0 AND ActiveStatus='Y' AND MacAdrs='O' AND TopupSeq>(Select TopupSeq FROM M_KitMaster WHERE KitID='" + objCustomerDetail.KitId.ToString() + "')";
                            // cmd = new SqlCommand();
                            // cmd.CommandText = query;
                            // cmd.Connection = SC;
                ...
                using (var entity = new InventoryEntities())
                {
                    ...
                    else if (objModel.BillType == "party")
                    {
                        ...
                        if (!string.IsNullOrEmpty(objModel.BillDateStr))
                        {
                            ...
                            BillDate = Convert.ToDateTime(objModel.BillDateStr);
                ...
                using (var entity = new InventoryEntities())
                {
                    ...
                    if (result != null)
                    {
                        ...
                        if (resultDetails != null)
                        {
                            ...
                            objDistributorModel.CompanyTel = resultDetails.MobileNo.ToString();
                            objDistributorModel.CompanyMail = resultDetails.E_MailAdd;
                        }
                        ...
                        foreach (var obj in objDistributorModel.objListProduct)
                        {
                            ...
                            obj.OrderDateStr = (obj.OrderDate ?? DateTime.Now).ToString("dd-MMM-yyyy");
                        }
        ...
        public async Task<ResponseDetail> SendMySMS(string message, string MobileNo)
        {
            ResponseDetail objResponse = new ResponseDetail();
            try
            {
                ...
                //Sc.Close();
                //Sc.Open();
                //string smsSenderID = "", smsUserNm = "", smsPassw = "";
                //using (SqlDataReader reader = cmd.ExecuteReader())
                //{
                //    while (reader.Read())
                //    {
                //        smsSenderID = reader["smsSenderID"].ToString();
                //        smsUserNm = reader["smsUserNm"].ToString();
                //        smsPassw = reader["smPass"].ToString();
                //    }
                //}
                ...
                {
                    objResponse.ResponseStatus = "OK";
                    objResponse.ResponseMessage = "SMS sent successfully!";
                }
                else
                {
                    objResponse.ResponseStatus = "FAILED";
        ...
        public async Task<ResponseDetail> SendOTP(string MobileNo, string TotalBillAmount)
        {
            ...
            objResponse = await Task.Run(() => SendMySMS(message, MobileNo));
                ...
                using (var entity = new InventoryEntities())
                {
                    objListPurchaseInvoice = (from r in entity.M_InwardMain
                                              ...
                                              select new PurchaseReport
                                              {
                                                  ...
                                                  TotalCGSTAmt = r.CGSTAmt.ToString(),
                                                  TotalSGSTAmt = r.SGSTAmt.ToString(),
                    ...
                    if (objListPurchaseInvoice.Count > 0)
                    {
                        ...
                        objListPurchaseInvoice[0].TotalTaxAmt = (decimal.Parse(objListPurchaseInvoice[0].TotalCGSTAmt) + decimal.Parse(objListPurchaseInvoice[0].TotalSGSTAmt) + decimal.Parse(objListPurchaseInvoice[0].TotalTaxAmt)).ToString();
        ...
        public ResponseDetail UpdateDeliveryDetails(UpdateDeliveryDetails obj)
        {
            ...
            try
            {
                ...
                foreach (var record in obj.DeliverDetailList)
                {
                    ...
                    //Sql += ";INSERT INTO VisionRoots..SendSMS (FormNo,Mobileno,SMS) ";
                    ...
                    //Sql += " UNION ";
                    //Sql += " Select b.Formno, b.Mobl, 'Package/Products, Inv No. '+ a.UserBillNo +' VRID No. '+ a.FCode +' have been dispatched by " + record.CourierName + ", AWB no. " + record.DocketNo + ", dated " + docketDate.ToString("dd-MMM-yyyy") + ".Team VR' FROM TrnBillmain a, VisionRoots..M_MemberMaster b WHERE a.Formno = b.Formno AND LEN(Cast(b.Mobl as varchar(15))) >= 10 AND(a.CourierName <> '" + record.CourierName + "' OR a.DocketNo <> '" + record.DocketNo + "')  AND a.UserBillNo = '" + record.BillNo + "'";
   ...
   " AND a.ActiveStatus = 'Y' AND a.RowStatus='Y' AND a.Qty>0";
        ...
        public List<PaymodeModel> GetMPaymodes()
        {
            ...
            try
            {
                ...
                using (var entity = new InventoryEntities())
                {
                    ...
                    string query = "Select*,LTRIM(RTRIM(AllBank)) as BankType FROM M_MPaymodeMaster WHERE ActiveStatus='Y' ORDER BY Paymode";
                    SqlCommand cmd = new SqlCommand();
                    cmd.CommandText = query;
        ...
        public List<OfferProduct> getExtraPVOfferProducts(int id, bool BuyProducts)
        {
            List<OfferProduct> objProductList = new List<OfferProduct>();
            try
            {
                using (var entity = new InventoryEntities())
                {
                    if (!BuyProducts)
                    {
                        objProductList = (from r in entity.VisionOfferProducts
                                          where r.OfferID == id && r.IsBuyProduct == "N"
                    ...
                    {
                        objProductList = (from r in entity.VisionOfferProducts
                                          where r.OfferID == id && r.IsBuyProduct == "Y"
                                          select new OfferProduct
                                          {
                                              AID = r.AID,
                                              ProductCode = r.ProdID,
        ...
        public List<OfferProduct> CheckForExtraPVOffer(DistributorBillModel objModel)
        {
            ...
            try
            {
                ...
                using (var entity = new InventoryEntities())
                {
                    ...
                    if (PVOffer != null)
                    {
                        List<OfferProduct> offerProduct = getExtraPVOfferProducts(PVOffer.OfferId, false);
